console.log("Hello Meta Description changer");
jQuery(document).ready(function ($) {
    ...
    const nonce = bulkMetaChangerToolData.nonce;
    ...
    NextBIT_uploadFileBtn.on('click', function (p) {
        ...
        if (fileInput.files.length > 0) {
            ...
            reader.onload = function (event) {
                ...
                SubmitButton.addEventListener('click', NextBIT_processMetaForSaving);
    ...
    function NextBIT_processMetaForSaving() {
        // Getting the table data
        const data = NextBIT_gettingMetaTableData();
        // Sorting data
    ...
    function NextBIT_gettingMetaTableData() {
        ...
        $.post(ajaxurl, {
            ...
            if (response.success) {
                ...
                if (Object.keys(data).length > 0) {
                    ...
                    let tableContainer = '<table class="record-table"><thead><tr><th>Index</th><th>Url</th><th>Meta Description</th><th>Date</th></tr></thead><tbody>';
                    for (const key in data) {
                        if (data.hasOwnProperty(key)) {
                            const section = data[key];
                            for (const item of section) {
                                tableContainer += `<tr><td>${index}</td><td>${key}</td><td>${item.md}</td><td>${item.d}</td></tr>`;
                                index++;
                            }
                        }
                    }
                    ...
                    const clearButton = $('<button>')
                        ...
                        .on('click', NextBIT_deleteMetaChangerFile);
    ...
    function NextBIT_downloadASCSVFile() {
        const fileName = 'Bulk-interlinking-tool_meta_data.csv';
        NextBIT_generateAndDownloadCSVFile(fileName);
    }

    async function NextBIT_deleteMetaChangerFile() {
        if (confirm('Are you certain you want to delete the stored Meta changer file and its data?')) {
            $.ajax({
